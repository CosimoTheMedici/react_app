...
const ManageProperty = () => {
  ...
  async function fetchPropertiesList() {
    try {
      const { data: fetchPropertiesResponses, status } = await axiosPrivate.get(
        `/api/v1/properties/`
      );
...
const TableModal = ({ stateChanger, stateChange }) => {
  const [propetyDetails, setPropetyDetails] = useState(
  ...
  const handleCloseModal = () => {
    ...
    //console.log(bodyElement.className); // Log the current body classes

    // Check if the modal-open class is present
    if (bodyElement.classList.contains("modal-open")) {
      // Remove modal-open class to allow body scrolling again
      bodyElement.classList.remove("modal-open");
    }
    //console.log(bodyElement.className);
    const allModalBackdrops = document.querySelectorAll(".modal-backdrop");
    //console.log(allModalBackdrops);
    ...
    if (modalBackdrop) {
      ...
      //console.log(modalBackdrop); // Do something with the backdrop if it exists
    } else {
  ...
  const handleSubmit = async (event) => {
    ...
    if (check === false) {
      ...
      try {
        const createPropertyResponse = await axiosPrivate.post(
    ...
    <>
      <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
        <div class="custom-actions-btns mb-5">
          ...
          >
            <i class="icon-person_add" style={{ fontSize: "18px" }}></i> Create
            Property
          </button>
        </div>
      </div>
      {isModalVisible && (
        <div
          class={`modal fade ${isModalVisible ? "show" : ""}`}
          id="customModalTwo"
          tabindex="-1"
          role="dialog"
